package org.openautomation.powermonitor.dao;
...
public class HS100 {
    ...
    public static final String COMMAND_GET_CURRENT_POWER_USAGE = "{\"emeter\":{\"get_realtime\":{}}}";
    ...
    public Map<String, String> getCurrentPowerUsage() throws IOException {
        Map<String, String> result = new HashMap<>();

        String jsonData = sendCommand(COMMAND_GET_CURRENT_POWER_USAGE);
        if(jsonData.length() > 0) {
            JsonObject jo = new JsonParser().parse(jsonData).getAsJsonObject();
            JsonObject systemInfo = jo.get("emeter").getAsJsonObject().get("get_realtime").getAsJsonObject();
            for(Map.Entry<String, JsonElement> entry : systemInfo.entrySet()) {
